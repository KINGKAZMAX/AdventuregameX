...
This repository is a **compilation of components under different licenses**. The
hub's own integration code (`src/`, `scripts/`, root config) is MIT — see
[`LICENSE`](LICENSE). Bundled components keep their own terms:

| Component | License |
| --- | --- |
| GameX hub (`src/`, `scripts/`) | MIT |
| `apps/game-boy` (Andrii Babintsev) | MIT |
| `apps/dammagotchi` | GNU AGPL-3.0 |
| WasmBoy emulator | GPL-3.0-or-later |
| Bundled homebrew ROMs | MIT / GPL / Zlib — see [`ATTRIBUTION.md`](apps/game-boy/public/roms/ATTRIBUTION.md) |

Because the combined work bundles AGPL-3.0 and GPL code, any redistribution must
also honour those copyleft terms.

Cartridge shells reproduce photos of real released cartridges for personal,
non-commercial display; sources are recorded in
`apps/game-boy/tools/pokemon-cartridges.json`. The playable games are the
open-licensed homebrew titles listed in
