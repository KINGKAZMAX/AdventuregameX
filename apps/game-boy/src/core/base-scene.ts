...
export default class BaseScene {
  ...
  private initRenderer(): void {
    ...
    renderer.setPixelRatio(this.getRenderPixelRatio());
  ...
  private initEffectsComposer(): void {
    const pixelRatio: number = this.getRenderPixelRatio();
  ...
  private getRenderPixelRatio(): number {
    return Math.max(1, Math.min(window.devicePixelRatio, SCENE_CONFIG.maxPixelRatio));
  }
  ...
  private showCopyrights(): void {
    ...
    copyrights.classList.remove('show');
  }
