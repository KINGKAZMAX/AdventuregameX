...
function assetPath(directory: string, filename: string): string {
  return `${assetBasePath}${directory}/${filename}`;
}
...
export default class Loader {
  ...
  private loadTextures(): void {
    ...
    TextureAssets.forEach((textureFilename: string) => {
      const textureFullPath: string = assetPath('textures', textureFilename);
  ...
  private loadModels(): void {
    ...
    ModelAssets.forEach((modelFilename: string) => {
      const modelFullPath: string = assetPath('models', modelFilename);
  ...
  private loadAudio(): void {
    ...
    AudioAssets.forEach((audioFilename: string) => {
      const audioFullPath: string = assetPath('audio', audioFilename);
