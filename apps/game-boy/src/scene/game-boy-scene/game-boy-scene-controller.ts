...
export default class GameBoyController {
  ...
  private cartridgeInfoPanel: CartridgeInfoPanel;
  private pendingCartridgeObject: THREE.Object3D | null;
  ...
  constructor(data: any) {
    ...
    this.cartridgeInfoPanel = new CartridgeInfoPanel();
    this.pendingCartridgeObject = null;
  ...
  public onPointerMove(x: number, y: number): void {
    ...
    // A drag past the threshold is a rotation intent, not a long-press: cancel.
    if (this.pendingCartridgeObject) {
      const dx = this.pointerPositionOnDown.x - x;
      const dy = this.pointerPositionOnDown.y - y;

      if (Math.hypot(dx, dy) > LONG_PRESS_MOVE_CANCEL_PX) {
        this.cancelPendingCartridge();
      }
    }
  ...
  public onPointerDown(x: number, y: number): void {
    ...
    // The open-source info card is modal; a press anywhere dismisses it and
    // does not fall through to the scene.
    if (this.cartridgeInfoPanel.isVisible()) {
      this.cartridgeInfoPanel.hide();
      return;
    }
    ...
    if (intersectObject) {
      ...
      // Cartridges: hold to read info, tap (on release) to insert. Defer the
      // insert so we can tell the two gestures apart.
      if (sceneObjectType === SCENE_OBJECT_TYPE.Cartridges && intersectObject.userData.isActive) {
        this.startPendingCartridge(intersectObject);
        return;
      }
  ...
  public onPointerUp(): void {
    // Released before the long-press fired → treat as a tap and insert.
    if (this.pendingCartridgeObject) {
      const cartridgeObject = this.pendingCartridgeObject;
      const wasLongPress = this.longPressFired;
      this.cancelPendingCartridge();

      if (!wasLongPress) {
        this.activeObjects[SCENE_OBJECT_TYPE.Cartridges].onPointerDown(cartridgeObject);
      }
    }
  ...
  private startPendingCartridge(cartridgeObject: THREE.Object3D): void {
    this.clearLongPressTimer();
    this.pendingCartridgeObject = cartridgeObject;
    this.longPressFired = false;
    ...
    this.longPressTimer = window.setTimeout(() => {
      ...
      const cartridgeType = cartridgeObject.userData['partType'];
      this.cartridgeInfoPanel.show(cartridgeType);
    }, LONG_PRESS_MS);
  }

  private cancelPendingCartridge(): void {
    this.clearLongPressTimer();
    this.pendingCartridgeObject = null;
    this.longPressFired = false;
  }
  ...
  private initIntroSignal(): void {
    ...
    // A press stops the reveal spin immediately; otherwise it settles on its
    // own so the Game Boy comes to rest facing front — where it never overlaps
    // the cartridge collection floating behind it.
    window.addEventListener('pointerdown', stopIntro);
    window.setTimeout(stopIntro, 3600);
  }
  ...
  private initDebugSignals(): void {
    ...
    this.gameBoyDebug.events.on('saveStateButtonClicked', () => { void this.onSaveStateButtonClicked(); });
    this.gameBoyDebug.events.on('loadStateButtonClicked', () => { void this.onLoadStateButtonClicked(); });
  }
  ...
  private onSoundsEnabledChanged(): void {
    ...
    this.games.onAudioEnabledChanged();
  }
