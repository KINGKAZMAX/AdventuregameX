...
const gameBoyAssetBasePath = import.meta.env.BASE_URL;
...
export default class GameBoy extends THREE.Group {
  ...
  private updateRotation(dt: number): void {
    ...
    if (this.isIntroActive) {
      ...
      // Gentle idle turntable spin: whenever the Game Boy is at rest and not
      // being dragged, keep it slowly rotating so it stays a showcase after the
      // intro ends. Cursor tilt (onPointerMove) rebuilds from this spinning base,
      // and a drag temporarily takes over, resuming the spin once it settles.
      if (this.isDefaultRotation
        && !this.isDragging
        && GAME_BOY_CONFIG.rotation.rotationCursorEnabled
        && GAME_BOY_CONFIG.rotation.debugRotationCursorEnabled
        && GAME_BOY_CONFIG.rotation.idleRotationSpeed > 0) {
        const angle: number = dt * 60 * GAME_BOY_CONFIG.rotation.idleRotationSpeed;
        const spin: THREE.Quaternion = new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0, 1, 0), angle);
        this.rotationQuaternion.premultiply(spin).normalize();
        this.rotationObject.quaternion.premultiply(spin).normalize();
      }
  ...
  private initZeldaIntroVideo(): void {
    ...
    videoElement.src = `${gameBoyAssetBasePath}video/zelda-intro.mp4`;
